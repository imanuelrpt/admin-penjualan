                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kode Konsumen</th>
                        ...
                        <?php if (count($konsumen) > 0): ?>
                            <?php foreach ($konsumen as $i => $k): ?>
                                <tr class="hover:bg-gray-50 transition duration-150">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><?= esc($k['kode_konsumen']) ?></td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900"><?= esc($k['nama']) ?></td>
                                    <td class="px-6 py-4 text-sm text-gray-500 max-w-xs truncate"><?= esc($k['alamat']) ?></td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><?= esc($k['no_hp']) ?></td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                        <div class="flex justify-center space-x-3">
                                            <a href="<?= base_url('/konsumen/' . $k['id']) ?>" class="text-blue-500 hover:text-blue-700 transition" title="Lihat Detail">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="<?= base_url('/konsumen/edit/' . $k['id']) ?>" class="text-green-500 hover:text-green-700 transition" title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <a href="<?= base_url('/konsumen/delete/' . $k['id']) ?>" class="text-red-500 hover:text-red-700 transition" title="Hapus" onclick="return confirm('Yakin ingin menghapus data ini?')">
